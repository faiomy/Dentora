...
ICON_PATTERNS = {
    ...
    "glossy": {"name": "المجسّم اللامع", "desc": "نفس الأشكال الملوّنة بس بانعكاس ضوئي فوقها كأنها مجسمات حقيقية"},
}
...
# ---------------- ستايلات موحّدة لعناصر الفورم (أزرار/قوائم منسدلة/شكبوكس/سويتش) ----------------
# customtkinter بيدّي أي زرار أو قايمة منسدلة أو شكبوكس أو سويتش ما اتحددلوش
# لون بنفسه لونه الأزرق الافتراضي بتاعه هو (مالوش أي علاقة بثيم البرنامج
# المختار). الدوال دي بترجع نفس ألوان الثيم الحالي (PRIMARY_LIGHT/ACCENT_BORDER)
# بدل الأزرق الافتراضي ده - وبترجعها كـ dict عشان تتحط مباشرة كـ **kwargs
# وقت إنشاء العنصر، فتاخد لون الثيم الفعلي وقت الإنشاء نفسه (مش لون قديم
# متجمّد وقت استيراد الملف) وتفضل متزامنة مع أي تغيير ثيم لاحق.
def optionmenu_colors(on_dark=False):
    """ألوان قايمة منسدلة (CTkOptionMenu). on_dark=True لو القايمة فوق خلفية
    غامقة (زي هيدر الصفحة) عشان جسمها يفضل فاتح وواضح فوقيها"""
    if on_dark:
        return dict(fg_color="#FFFFFF", text_color=TEXT_DARK,
                    button_color=ACCENT_BORDER, button_hover_color=darken_color(ACCENT_BORDER),
                    dropdown_fg_color=CARD_BG, dropdown_text_color=TEXT_DARK,
                    dropdown_hover_color=lighten_color(PRIMARY_LIGHT, 0.75))
    return dict(fg_color=CARD_BG, text_color=TEXT_DARK,
                button_color=PRIMARY_LIGHT, button_hover_color=ACCENT_BORDER,
                dropdown_fg_color=CARD_BG, dropdown_text_color=TEXT_DARK,
                dropdown_hover_color=lighten_color(PRIMARY_LIGHT, 0.75))


def checkbox_colors():
    """ألوان شكبوكس (CTkCheckBox): لون تعليمه (وقت الاختيار) بلون الثيم
    الأساسي بدل الأزرق الافتراضي"""
    return dict(fg_color=PRIMARY_LIGHT, hover_color=ACCENT_BORDER, checkmark_color="#FFFFFF")


def switch_colors():
    """ألوان سويتش (CTkSwitch): لون المسار وقت التفعيل بلون الثيم الأساسي"""
    return dict(progress_color=PRIMARY_LIGHT, button_hover_color=ACCENT_BORDER)


def button_neutral_colors():
    """ألوان زرار ثانوي/مساعد صغير (زي أسهم التنقّل بين الشهور، أو زرار
    "إلغاء" بجانب زرار أساسي) - محايدة من لوحة الثيم بدرجة فاتحة عشان تبقى
    واضح إنها أقل أهمية من الزرار الأساسي (accent) من غير ما تفضل بلون أزرق
    ثابت مالوش علاقة بالثيم"""
    return dict(fg_color=BG_MAIN, text_color=TEXT_DARK,
                hover_color=lighten_color(PRIMARY_LIGHT, 0.75),
...
class GlassIconButton:
    ...
    def __init__(self, parent, text, command, width=48, height=34, accent_color=None,
                 canvas_bg=None, active=False, font=None, corner_radius=10, icon_image=None,
                 icon_scale=0.62):
        import tkinter as tk
        ...
        self.text = text
        # أيقونة ملوّنة اختيارية (PIL Image بصيغة RGBA) بتترسم فوق خلفية
        # الزرار بدل النص - مفيدة لما نحتاج ألوان حقيقية للأيقونة (Canvas
        # text في Tk بيرسم بلون واحد بس (fill)، حتى لو الحرف نفسه إيموجي
        # ملوّن أصلاً - مش بيدعم رسم إيموجي متعدد الألوان). لو من غير ما
        # تتحدد، السلوك القديم (نص/إيموجي أحادي اللون) فاضل شغال زي ما هو
        self.icon_image = icon_image
        # نسبة حجم الأيقونة من أصغر ضلع في الزرار (0.62 الافتراضي القديم
        # زي ما هو لأي استخدام تاني في البرنامج مبيحددش قيمة مختلفة صراحة،
        # فمفيش أي تأثير على الأزرار التانية)
    ...
    def _glass_pil(self, active, hover):
        ...
        if self.icon_image is not None:
            ...
            # بنفس مقياس الـ supersampling (scale) المستخدم في رسم الخلفية
            # عشان تطلع ناعمة الحواف زي باقي الزرار
            icon_target = int(min(W, H) * self.icon_scale)
            icon_resized = self.icon_image.resize((icon_target, icon_target), Image.LANCZOS)
            ix = (W - icon_target) // 2
            iy = (H - icon_target) // 2
    ...
    def _redraw(self):
        ...
        if self.icon_image is not None:
            # الأيقونة الملوّنة اتلزقت خلاص جوه صورة الخلفية نفسها في
            # _glass_pil، فمفيش داعي نرسم نص/حرف تاني فوقها
            return
        text_id = self.canvas.create_text(self.width / 2, self.height / 2, text=self.text,
                                           fill=self._text_color(), font=self.font,
                                           anchor="center", tags="label")
        # تصحيح دقيق لمركز الرسمة: بعض حروف الإيموجي عندها مسافات فارغة غير
        # متماثلة جوه صندوق الخط بتاعها (font metrics)، فمركز الرسمة الفعلي
        # (bounding box) بيبقى مش نفس المركز اللي بيحسبه Tkinter افتراضيًا.
        # بناخد الـ bbox الحقيقي للرسمة المرسومة ونزحزحها لحد ما مركزها
        # الفعلي يطابق مركز المربع بالظبط، أيًا كان شكل الحرف/الإيموجي
        bbox = self.canvas.bbox(text_id)
        if bbox:
            actual_cx = (bbox[0] + bbox[2]) / 2
            actual_cy = (bbox[1] + bbox[3]) / 2
            dx = self.width / 2 - actual_cx
            dy = self.height / 2 - actual_cy
            # حماية إضافية: بعض حروف الإيموجي المركّبة (أكتر من حرف يونيكود
            # واحد، زي رموز فيها "variation selector") بيتحسب لها bbox غير
            # دقيق على بعض الأجهزة/الخطوط، فمحاولة تصحيح المركز ممكن
            # تزحزحها لحد برّه حدود المربع وتتقص. عشان كده بنمنع أي إزاحة
            # ممكن تطلّع جزء من الرسمة خارج حدود الـ Canvas
            new_left, new_right = bbox[0] + dx, bbox[2] + dx
            new_top, new_bottom = bbox[1] + dy, bbox[3] + dy
            if new_left < 0:
                dx -= new_left
            if new_right > self.width:
                dx -= (new_right - self.width)
            if new_top < 0:
                dy -= new_top
            if new_bottom > self.height:
                dy -= (new_bottom - self.height)
            if dx or dy:
...
def confirm_dialog(parent, message, on_confirm, title="تأكيد",
    ...
    # الزرارين بقوا أصغر وباخدين حوالي ربع عرض السطر بس لكل واحد، وكل
    # الكتابة بيضا في الاتنين (تأكيد/إلغاء) بدل زرار الإلغاء الشفاف زي
    # ما كان قبل كده
    BTN_W, BTN_H = 84, 32
