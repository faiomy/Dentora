...
def verify_password(password: str, stored_hash: str) -> bool:
    """Verifies a password against a stored PBKDF2 hash or legacy plaintext string."""
    if not stored_hash:
        return False
    if not password:
        password = ""
...
def init_db():
    ...
    if cur.fetchone()[0] == 0:
        ...
        cur.execute("""
            ...
            VALUES ('admin', ?, 'مدير العيادة', 'manager')
        """, (default_admin_hash,))
    else:
        # ترقية كلمات المرور القديمة المخزنة كنص صريح إلى هاش آمن تلقائيًا
        cur.execute("SELECT id, password FROM users")
        user_rows = cur.fetchall()
        for u_id, u_pw in user_rows:
            if u_pw and not str(u_pw).startswith("pbkdf2_sha256$"):
                new_h = hash_password(str(u_pw))
                cur.execute("UPDATE users SET password = ? WHERE id = ?", (new_h, u_id))
    ...
    # ملحوظة مفصّلة على سن معين (تاريخ + طبيب + نص) - منفصلة عن الملاحظة
    # الحرة البسيطة في tooth_chart.notes، وبتظهر لها علامة مميزة فوق/تحت
    # السن في الشارت. سن واحد = ملحوظة واحدة (آخر تحديث بيستبدل اللي قبله)
    cur.execute("""
    ...
    # الألوان الافتراضية لكل نوع علاج (البنود الأساسية الجاهزة بقت ألوانها
    # ثابتة غير قابلة للتخصيص من صفحة الإجراءات - مصدرها الموحّد
    # tooth_symbols.BUILTIN_TREATMENT_COLORS)
    DEFAULT_TREATMENT_COLORS = dict(tooth_symbols.BUILTIN_TREATMENT_COLORS)
    ...
    # البنود الأساسية الجاهزة (تسوس/حشو/جذر/دعامة/زراعة/جير/خلع) ألوانها ثابتة
    # دايمًا وغير قابلة للتخصيص من صفحة الإجراءات - بتتفرض هنا أخيرًا (بعد أي
    # تصفير سابق بسبب وجود أنواع فرعية مثلًا) عشان تكسب دايمًا على أي منطق
    # تاني بيمسح اللون، ويتلغي أي تخصيص سابق للون كان محفوظ من نسخة أقدم
    for tkey, default_color in tooth_symbols.BUILTIN_TREATMENT_COLORS.items():
        cur.execute(
            "UPDATE treatment_prices SET color = ? WHERE treatment_key = ?",
            (default_color, tkey))
    ...
    # الخصم بقى بيتسجل مباشرة على كل بند مستحق (transactions) وبقى قابل
    # للتعديل يدويًا من تاب الحسابات - "المستحق" الفعلي لكل بند = amount
    # - discount_amount، وهو ده اللي بيتحسب عليه رصيد المريض
...
# ---------------- ملحوظات مفصّلة على السن (تاريخ + طبيب + نص) ----------------

def upsert_tooth_annotation(patient_id, tooth_number, note_date, doctor_name, note_text):
    """بتسجل/تحدّث الملحوظة المفصّلة الخاصة بسن معين (سن واحد = ملحوظة واحدة
    بس - أي حفظ جديد بيستبدل القديمة)"""
    conn = get_connection()
    conn.execute("""
        INSERT INTO tooth_annotations (patient_id, tooth_number, note_date, doctor_name, note_text, updated_at)
        VALUES (?, ?, ?, ?, ?, ?)
        ON CONFLICT(patient_id, tooth_number)
        DO UPDATE SET note_date = excluded.note_date, doctor_name = excluded.doctor_name,
                      note_text = excluded.note_text, updated_at = excluded.updated_at
    """, (patient_id, tooth_number, note_date, doctor_name, note_text,
          datetime.now().strftime("%Y-%m-%d %H:%M")))
    conn.commit()
    conn.close()


def get_tooth_annotation(patient_id, tooth_number):
    conn = get_connection()
    row = conn.execute(
        "SELECT * FROM tooth_annotations WHERE patient_id = ? AND tooth_number = ?",
        (patient_id, tooth_number)).fetchone()
    conn.close()
    return dict(row) if row else None


def get_tooth_annotations_map(patient_id):
    """بترجع dict: {tooth_number: {...}} لكل الأسنان اللي ليها ملحوظة مفصّلة
    مسجلة للمريض ده - المستخدمة لرسم العلامة المميزة على الشارت"""
    conn = get_connection()
    rows = conn.execute(
        "SELECT * FROM tooth_annotations WHERE patient_id = ?", (patient_id,)).fetchall()
    conn.close()
    return {r["tooth_number"]: dict(r) for r in rows}


def delete_tooth_annotation(patient_id, tooth_number):
    conn = get_connection()
    conn.execute(
        "DELETE FROM tooth_annotations WHERE patient_id = ? AND tooth_number = ?",
...
def get_active_tooth_conditions(patient_id):
    ...
    """
    records = get_treatment_records(patient_id)  # بقت الأقدم أولًا (ASC)
    records_asc = records  # بترتيب زمني تصاعدي بالفعل، مفيش داعي نعكسها تاني
...
def update_treatment_price(price_list_id, treatment_key, label, price, commission_percent=0,
    ...
    conn = get_connection()
    # البنود الأساسية الجاهزة (تسوس/حشو/جذر/دعامة/زراعة/جير/خلع) ألوانها ثابتة
    # دايمًا مهما كان اللي اتبعت هنا - مفيش تخصيص للون بتاعها إطلاقًا
    if treatment_key in tooth_symbols.BUILTIN_TREATMENT_COLORS:
        color = tooth_symbols.BUILTIN_TREATMENT_COLORS[treatment_key]
    if color is None:
...
def add_manual_treatment_record(patient_id, description, price, treatment_date=None):
    """إضافة "مستحق" يدوي مباشرة من تاب الحسابات (مش جاي من شارت الأسنان) -
    بتتسجل كسجل معالجة (tooth_number=None) عشان تظهر في نفس الجدول
    الموحّد، ومعاها حركة "مستحق" (charge) مرتبطة بيها في transactions
    عشان تدخل في حساب المريض وحسابات العيادة الرئيسية زي أي معالجة تانية."""
    record_id = add_treatment_record(
        patient_id, tooth_number=None, treatment_key=None,
        treatment_label=description or "مستحق يدوي", price=price,
        treatment_date=treatment_date)
    if price and price > 0:
        add_transaction(patient_id, "charge", price, description=description or "مستحق يدوي",
                         tx_date=treatment_date, related_treatment_id=record_id)
    return record_id
...
def get_treatment_records(patient_id):
    ...
    rows = conn.execute("""
        ...
        WHERE tr.patient_id = ?
        ORDER BY tr.treatment_date ASC, tr.id ASC
    """, (patient_id,)).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def update_treatment_record_fields(record_id, **fields):
    """تحديث حقل واحد أو أكتر من سجل معالجة (تعديل مباشر من الجدول الموحّد
    في تاب الحسابات بتاع المريض). بيقبل بس الحقول الموجودة في
    TREATMENT_RECORD_EDITABLE_FIELDS حماية من تعديل حقول حساسة زي
    patient_id بالغلط.
    ربط تلقائي في الاتجاهين مع صفحة الحسابات: أي تعديل في القيمة/الخصم هنا
    بيتحدّث معاه الحركة المالية (charge) المرتبطة بنفس المعالجة، وأي تعديل
    في "المدفوع" بيتحدّث/يتسجّل معاه حركة "دفعة" (payment) مرتبطة بنفس
    المعالجة كمان - عشان إجمالي حساب المريض في كارت الملخص وصفحة حسابات
    العيادة الرئيسية يفضلوا مطابقين تمامًا لأي تعديل يتم من هنا."""
    ...
    # (1) القيمة/الخصم -> بتتحدث في الحركة المالية (charge) المرتبطة
    if "price" in fields or "discount_amount" in fields:
        tx_row = conn.execute(
            "SELECT id FROM transactions WHERE related_treatment_id = ? AND tx_type = 'charge'",
            (record_id,)).fetchone()
        if tx_row:
            tx_updates = {}
            if "price" in fields:
                tx_updates["amount"] = fields["price"]
            if "discount_amount" in fields:
                tx_updates["discount_amount"] = fields["discount_amount"]
            tx_set_clause = ", ".join(f"{c} = ?" for c in tx_updates)
            conn.execute(f"UPDATE transactions SET {tx_set_clause} WHERE id = ?",
                         list(tx_updates.values()) + [tx_row["id"]])

    # (2) المدفوع -> بيتسجل/يتحدّث كحركة "دفعة" مرتبطة بنفس المعالجة عشان
    # يدخل في إجمالي المدفوع بتاع المريض (ما فيش جدول "سجل دفعات" منفصل
    # النهاردة، فده المصدر الوحيد لتسجيل مدفوعات كل معالجة)
    if "paid_amount" in fields:
        new_paid = float(fields["paid_amount"] or 0)
        pay_row = conn.execute(
            "SELECT id FROM transactions WHERE related_treatment_id = ? AND tx_type = 'payment'",
            (record_id,)).fetchone()
        if pay_row:
            if new_paid <= 0:
                conn.execute("DELETE FROM transactions WHERE id = ?", (pay_row["id"],))
            else:
                conn.execute("UPDATE transactions SET amount = ? WHERE id = ?",
                             (new_paid, pay_row["id"]))
        elif new_paid > 0:
            rec = conn.execute(
                "SELECT patient_id, treatment_date FROM treatment_records WHERE id = ?",
                (record_id,)).fetchone()
            if rec:
                conn.execute("""
                    INSERT INTO transactions
                        (patient_id, tx_type, amount, description, tx_date, related_treatment_id)
                    VALUES (?, 'payment', ?, ?, ?, ?)
                """, (rec["patient_id"], new_paid, "دفعة معالجة",
                      rec["treatment_date"] or datetime.now().strftime("%Y-%m-%d"), record_id))
...
def delete_transaction(tx_id):
    """بتمسح حركة مالية واحدة. لو الحركة دي ناتجة أصلاً عن تسجيل معالجة من
    شارت الأسنان (ليها related_treatment_id)، بنمسح سجل المعالجة نفسه كمان
    (مش بس الحركة المالية) عشان صفحة الحسابات وشارت الأسنان يفضلوا متطابقين
    مع بعض دايمًا - من غير كده كانت المعالجة بتفضل ظاهرة في الشارت وكأن
    حاجة متسجلاش، رغم إن حسابها اتلغى من صفحة الحسابات"""
    conn = get_connection()
    row = conn.execute(
        "SELECT related_treatment_id FROM transactions WHERE id = ?", (tx_id,)).fetchone()
    related_treatment_id = row["related_treatment_id"] if row else None
    conn.close()

    if related_treatment_id:
        delete_treatment_record(related_treatment_id)
    else:
        conn = get_connection()
...
def get_patient_balance(patient_id):
    """المستحق الفعلي لكل بند = amount - discount_amount، والرصيد كله بيتحسب
    على أساس المستحق ده (بعد الخصم) مش المبلغ الخام قبل الخصم"""
    conn = get_connection()
    row = conn.execute(
        "SELECT COALESCE(SUM(amount),0) AS amt, COALESCE(SUM(discount_amount),0) AS disc "
        "FROM transactions WHERE patient_id = ? AND tx_type = 'charge'",
        (patient_id,)).fetchone()
    charges = row["amt"] - row["disc"]
    payments = conn.execute(
...
def get_patient_total_discount(patient_id):
    """إجمالي الخصومات المسجّلة مباشرة على بنود سجل المعالجات (transactions.
    discount_amount) - مستخدمة في كارت ملخص الحسابات بتاع تاب الحسابات"""
    conn = get_connection()
    total = conn.execute(
        "SELECT COALESCE(SUM(discount_amount),0) FROM transactions "
        "WHERE patient_id = ? AND tx_type = 'charge'",
        (patient_id,)).fetchone()[0]
    conn.close()
    return total


def update_transaction_amount(tx_id, new_amount):
    conn = get_connection()
    conn.execute("UPDATE transactions SET amount = ? WHERE id = ?", (new_amount, tx_id))
    # لو الحركة دي ناتجة عن معالجة مسجَّلة في "سجل المعالجات"، لازم نحدّث
    # القيمة (price) هناك كمان عشان الجدول الموحّد في تاب الحسابات وشارت
    # الأسنان يفضلوا متطابقين دايمًا - وبنحافظ على نسبة الخصم الحالية
    row = conn.execute(
        "SELECT related_treatment_id FROM transactions WHERE id = ?", (tx_id,)).fetchone()
    related_id = row["related_treatment_id"] if row else None
    if related_id:
        rec = conn.execute(
            "SELECT discount_percent FROM treatment_records WHERE id = ?", (related_id,)).fetchone()
        percent = float(rec["discount_percent"] or 0) if rec else 0
        new_discount = round(new_amount * percent / 100, 2)
        conn.execute(
            "UPDATE treatment_records SET price = ?, discount_amount = ? WHERE id = ?",
            (new_amount, new_discount, related_id))
        conn.execute("UPDATE transactions SET discount_amount = ? WHERE id = ?",
                     (new_discount, tx_id))
    conn.commit()
    conn.close()


def update_transaction_discount(tx_id, new_discount):
    conn = get_connection()
    conn.execute("UPDATE transactions SET discount_amount = ? WHERE id = ?",
                 (new_discount, tx_id))
    # نفس فكرة update_transaction_amount، لكن هنا القيمة (amount) بتفضل
    # ثابتة وبنعيد حساب نسبة الخصم% في سجل المعالجات المرتبط
    row = conn.execute(
        "SELECT related_treatment_id, amount FROM transactions WHERE id = ?", (tx_id,)).fetchone()
    related_id = row["related_treatment_id"] if row else None
    if related_id:
        price = float(row["amount"] or 0)
        new_percent = round((new_discount / price) * 100, 2) if price > 0 else 0
        conn.execute(
            "UPDATE treatment_records SET discount_amount = ?, discount_percent = ? WHERE id = ?",
            (new_discount, new_percent, related_id))
    conn.commit()
    conn.close()
...
def get_treatment_record(record_id):
    conn = get_connection()
    row = conn.execute("SELECT * FROM treatment_records WHERE id = ?", (record_id,)).fetchone()
    conn.close()
    return dict(row) if row else None
...
def get_visits(patient_id):
    conn = get_connection()
    rows = conn.execute(
        "SELECT * FROM visits WHERE patient_id = ? ORDER BY visit_date ASC, id ASC",
        (patient_id,)).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
PERMISSION_LABELS = {
    ...
    "manage_prices": "إدارة الإجراءات الطبية وقوائم الأسعار",
    "manage_settings": "إدارة إعدادات العيادة",
    "manage_users": "إدارة المستخدمين والصلاحيات",
    "manage_expenses": "إدارة المصروفات والموردين",
    "view_clinic_accounts": "عرض تقرير حسابات العيادة (الإيرادات/المصروفات)",
    "manage_staff": "إدارة طاقم العمل (أطباء/مساعدين/سكرتارية/خدمات مساعدة)",
...
def authenticate_user(username, password):
    ...
    if verify_password(password, stored_hash):
        # Auto-rehash legacy plaintext password on successful login
        if stored_hash and not stored_hash.startswith("pbkdf2_sha256$"):
            try:
                new_h = hash_password(password)
                conn2 = get_connection()
                conn2.execute("UPDATE users SET password = ? WHERE id = ?", (new_h, user_dict["id"]))
                conn2.commit()
                conn2.close()
                user_dict["password"] = new_h
            except Exception:
                pass
        return user_dict
    return None
...
def add_user(username, password, full_name, role, phone="", specialty="", work_days=""):
    ...
    cur = conn.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (username, pw_hash, full_name, role, phone, specialty, work_days))
