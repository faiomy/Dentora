...
class ToothChart(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        # ملاحظة حرة على السن المحدد
        note_row = ctk.CTkFrame(self, fg_color="transparent")
        note_row.pack(fill="x", padx=16, pady=(0, 10))
        ctk.CTkLabel(note_row, text="ملاحظة على السن المحدد:", font=theme.FONT_NORMAL,
                     text_color=theme.TEXT_MUTED).pack(side="right", padx=(0, 8))
        self.note_entry = ctk.CTkEntry(note_row, height=34, justify="right",
                                        font=theme.FONT_NORMAL)
        self.note_entry.pack(side="right", fill="x", expand=True, padx=(8, 0))
        ctk.CTkButton(note_row, text="حفظ الملاحظة", width=110, height=34, fg_color=theme.PRIMARY_LIGHT,
                      command=self._save_note).pack(side="right", padx=(8, 0))
    ...
    def _select_tooth(self, tooth_num):
        ...
        note = db.get_tooth_notes(self.patient_id).get(active_num, "")
    ...
    def _save_note(self):
        if self.selected_tooth is None:
            return
        active_num = self._active_number(self.selected_tooth)
        db.set_tooth_note(self.patient_id, active_num, self.note_entry.get().strip())
