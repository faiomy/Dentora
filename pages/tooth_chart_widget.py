...
# الأضراس السفلية اللي المفروض تاخد 2 cusps بس (مش 3) - الرحوية التانية
# والتالتة (العقل) يمين وشمال. باقي الأضراس السفلية (زي الأولى) فاضلة 3 cusps
FEW_CUSP_LOWER_MOLARS = {47, 48, 37, 38}
...
def _tooth_geometry(category, upper, jaw_upper, tooth_num=None):
    ...
    if category == "incisor":
        ...
        TWO_CUSP_CROWN = [(7, 18), (15, 6), (25, 14), (35, 6), (43, 18), (43, 28), (7, 28)]
        THREE_CUSP_CROWN = [(7, 18), (13, 6), (19, 14), (25, 6), (31, 14), (37, 6), (43, 18), (43, 28), (7, 28)]
        if jaw_upper:
            # الضرس العلوي الحقيقي بيه cusp اتنين بس (لثوي وحنكي) مش 3
            crown = TWO_CUSP_CROWN
            # الضرس العلوي الحقيقي بيه 3 جذور (لثوي وفلحي أمامي وفلحي خلفي) -
            # التلاتة جذور دلوقتي بنفس السُمك بالظبط (الجذر الأوسط اتقلل
            # سُمكه بردو عشان يطابق الجذرين التانيين بعد تقليل سُمكهم بمقدار
            # الربع 1/4 عن سُمكهم الأصلي القديم)
            roots = [
                [(9.25, GUM_Y), (16.75, GUM_Y), (13, 76)],
                [(21.25, GUM_Y), (28.75, GUM_Y), (25, 80)],
                [(33.25, GUM_Y), (40.75, GUM_Y), (37, 76)],
            ]
            ...
            # جدًا من خط اللثة (مش نازلين ملتصقين لنص الجذر). الجذر الأوسط
            # (الحنكي) كان بيبان صغير قوي عن التانيين رغم إن عرضه عند اللثة
            # متساوي، لأن خطه كان شبه مستقيم من غير أي "بطن" - فبقى ليه دلوقتي
            # بطن واضح على الجنبين (بنفس مدى بروز البطن بتاع الجذرين التانيين)
            # عشان يبان بنفس حجمهم فعلاً مش بس بنفس عرض قاعدته
            outline = [
                (7, 18), (15, 6), (25, 14), (35, 6), (43, 18),
                (43, 28),
                (45.25, 36), (44.5, 52), (40.75, 68), (37.75, 78),
                (34, 64), (32.5, 50), (31.75, 40),
                (30.5, 50), (28, 64), (25, 80),
                (22, 64), (19.5, 50), (19, 40),
                (16, 50), (14.5, 64), (12.25, 78),
                (8.5, 68), (5.5, 52), (4.75, 36),
                (7, 28),
            ]
        else:
            # الضرس السفلي الحقيقي بيه جذرين (أمامي وخلفي)، وبيبقى بيه 2
            # cusp بس (مش 3) للضرس التاني والتالت (رقمهم بينتهي بـ7 أو 8) -
            # زي 47/48/37/38، أما الضرس الأول (بينتهي بـ6) فبيفضل 3 cusps
            crown = TWO_CUSP_CROWN if tooth_num in FEW_CUSP_LOWER_MOLARS else THREE_CUSP_CROWN
            # سُمك الجذرين اتقلل بمقدار الربع 1/4 عن سُمكهم الأصلي القديم
            # (بنفس طريقة تقليل جذور الضرس العلوي بالظبط) - نفس شكل الجذر
            # تمامًا بس أنحف
            roots = [
                [(11.625, GUM_Y), (21.375, GUM_Y), (16.125, 76)],
                [(28.625, GUM_Y), (38.375, GUM_Y), (34.375, 76)],
            ]
            outline = [
                (7, 18), (13, 6), (19, 14), (25, 6), (31, 14), (37, 6), (43, 18),
                (43, 28),
                (43.625, 38), (42.875, 56), (38.375, 72), (34.625, 82),
                (30.125, 68), (27.875, 54), (27.125, 42),
                (21.375, 54), (19.125, 68), (15.375, 82),
                (11.625, 72), (7.125, 56), (6.375, 38),
                (7, 28),
            ]
...
class ToothChart(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(doctor_row, text="الطبيب المعالج:", font=theme.FONT_NORMAL,
                     text_color=theme.TEXT_MUTED).pack(side="right", padx=(0, 20))
        ...
        # صندوق منسدلة الأطباء: ارتفاع أقل بمقدار الرُبع (34 -> 26) مع فونت
        # أكبر وهوامش رأسية داخلية أضيق (2-3 بيكسل)، وتظليل فاتح جدًا من
        ...
        self.doctor_menu = ctk.CTkOptionMenu(
            doctor_box, values=doctor_names, width=200, height=26,
            font=(theme.CONTENT_FONT_FAMILY, 15),
            dropdown_font=(theme.CONTENT_FONT_FAMILY, 15),
            fg_color=DOCTOR_BOX_LIGHT, text_color="#000000",
            button_color=theme.lighten_color(theme.PRIMARY_LIGHT, 0.7),
            button_hover_color=theme.ACCENT_BORDER,
            dropdown_fg_color=theme.CARD_BG, dropdown_text_color="#000000",
            dropdown_hover_color=theme.lighten_color(theme.PRIMARY_LIGHT, 0.75))
        self.doctor_menu.set(default_doctor)
        self.doctor_menu.pack(padx=2, pady=2)
    ...
    def _build_toolbar(self):
        ...
        كل زرار بياخد أصغر عرض يستوعب نص اسمه بس (مش عرض ثابت موحّد
        لكل الأزرار)، عشان الأسماء القصيرة تاخد مساحة أقل والطويلة تاخد
        المساحة اللي محتاجاها بس"""
        ...
        # كل أزرار المعالجات بلون واحد موحّد فاتح جدًا من الثيم المطبّق فعليًا
        # (مش لون مختلف لكل معالجة)، مع إطار أسود حواليها
        UNIFORM_BTN_BG = theme.lighten_color(theme.PRIMARY_LIGHT, 0.85)
        UNIFORM_BTN_HOVER = theme.lighten_color(theme.PRIMARY_LIGHT, 0.7)
        measure_font = tkfont.Font(family=theme.FONT_NORMAL[0], size=theme.FONT_NORMAL[1])
        for status_key in ordered_keys:
            label = self.prices_cache[status_key]["label"]
            text_w = measure_font.measure(label)
            btn_width = max(56, text_w + 28)  # أصغر حجم يستوعب النص + هامش بسيط حواليه
            ctk.CTkButton(
    ...
    def _delete_condition_multi(self, record_ids):
        ...
        أكتر من سن محدد في نفس الوقت) - وبتمسح معاها حصة كل سن من الحساب
        المالي (كل سجل بقى ليه حركته المالية الخاصة بيه، فمسحه بيمسح حصته
        هو بس ومفيش تأثير على باقي الأسنان اللي مش محددة للحذف"""
        count = len(record_ids)
        if not messagebox.askyesno(
                "تأكيد الحذف",
                f"هل تريد حذف هذه المعالجة من {count} سن نهائيًا؟\n"
                "سيتم حذفها من شارت الأسنان ومن حساب المريض المالي معًا.",
                parent=self):
            return
    ...
    def _render_details(self):
        ...
        if not conditions:
            if presence == teeth_time.PRESENT:
                ctk.CTkLabel(self.details_frame, text="لا توجد معالجات مسجلة على هذا السن - سليم",
    ...
    def _render_details_multi(self):
        ...
        if not groups:
            ctk.CTkLabel(self.details_frame,
                         text="لا توجد معالجات مسجلة على الأسنان المحددة",
                         font=theme.FONT_SMALL, text_color=theme.TEXT_MUTED).pack(anchor="e", pady=2)
            return
    ...
    def _draw_tooth(self, tooth_num):
        ...
        crown_pts, root_polys, outline_pts = _tooth_geometry(category, upper, jaw_upper, tooth_num=active_num)
        if is_primary:
        ...
        # علاج العصب - نفس شكل الجذر بالظبط (تشريحيًا) لكن بتصغير أفقي 60%
        # (نفس طول الجذر تمامًا، بس أنحف)، وفي منتصف المسافة الأفقية بين
        # حافتي الجذر اليمين والشمال - بيترسم في كل جذر من جذور السن نفسها
        # (ضاحك = جذر واحد، ضرس سفلي = جذرين، ضرس علوي = 3 جذور - بالظبط
        # بعدد الجذور المرسومة)
        if "root_canal" in conditions and "implant" not in conditions:
            color = self._color_for("root_canal", conditions)
            ROOT_CANAL_WIDTH_RATIO = 0.6
            for root in root_polys:
                # مركز الجذر أفقيًا = منتصف المسافة بين حافته اليمين والشمال
                # عند خط اللثة (أول نقطتين في مضلع الجذر)
                root_cx = (root[0][0] + root[1][0]) / 2
                narrow_root = [
                    (root_cx + (x - root_cx) * ROOT_CANAL_WIDTH_RATIO, y)
                    for x, y in root
                ]
                canvas.create_polygon(*[c for p in narrow_root for c in p],
                                       fill=color, outline=color, smooth=False)
    ...
    def _draw_ghost_tooth(self, canvas, tooth_num, upper, presence):
        ...
        crown_pts, root_polys, outline_pts = _tooth_geometry(category, draw_upper, jaw_upper, tooth_num=tooth_num)
    ...
    def _draw_treatment_overlay(self, canvas, upper, crown_pts, cx, coverage_colors, post_color,
        ...
        # الجير - خط مستقيم متصل عند خط اللثة (بدل النقاط المتقطعة القديمة)
        if calculus_color:
            gum_y = 40 if upper else TOOTH_H - 40
            rgba = self._hex_to_rgba(calculus_color, alpha)
            line_half = 11
            draw.line([(cx - line_half) * scale, gum_y * scale,
                       (cx + line_half) * scale, gum_y * scale],
                      fill=rgba, width=max(1, round(3 * scale)))
        ...
        if len(self.selected_teeth) > 1:
            ...
            self.selection_label.configure(
                text=f"محدد {len(self.selected_teeth)} سن معًا: {teeth_txt}   -   "
                     f"انقر نوع العلاج لإضافته إلى جميع هذه الأسنان معًا")
            self._render_details()
    ...
    def _apply_treatment(self, status_key, surfaces=None, variant=None, teeth=None, unit_count=None):
        ...
        عدد الأسنان، مش بالضرورة نفس عدد الأسنان المحددة فعليًا)، وبتتقسم
        بالتساوي على كل سن من الأسنان المحددة مع حركة مالية خاصة بكل سن على
        حدة (بدل حركة واحدة مشتركة مربوطة بسن واحد بس زي ما كان قبل كده) -
        عشان لو اتحذف سن واحد من المجموعة، تتحذف حصته من الحساب هو بس
        ويفضل الباقي زي ما هو من غير ما يتأثر بيه"""
        ...
        # يبانوا على كل سن منهم في الشارت والتلميح)، والتكلفة والعمولة
        # بتتقسم بالتساوي بينهم (مش كلها على أول سجل بس) وكل سجل بياخد
        # حركته المالية الخاصة بيه (related_treatment_id بتاعه هو، مش سجل
        # تاني) - عشان حذف أي سن يمسح حصته بس من غير ما يأثر على الباقي
        n_teeth = len(teeth)
        base_price = round(total_price / n_teeth, 2) if n_teeth else 0
        base_commission = round(total_commission / n_teeth, 2) if n_teeth else 0
        price_running, commission_running = 0.0, 0.0
        ...
        for i, tooth_num in enumerate(teeth):
            ...
            if i == n_teeth - 1:
                # آخر سن بياخد الباقي بالظبط (مش نفس نصيب المتوسط) عشان
                # مجموع كل الأنصبة يفضل يساوي التكلفة الكلية بالظبط حتى مع
                # أي فرق تقريب بسيط في القسمة
                record_price = round(total_price - price_running, 2)
                record_commission = round(total_commission - commission_running, 2)
            else:
                record_price = base_price
                record_commission = base_commission
                price_running += base_price
                commission_running += base_commission
            ...
            if record_price > 0:
                if is_bulk:
                    desc = f"{label} - سن رقم {active_num} (من ضمن {len(teeth)} سن، محسوبة كـ {unit_count} سن)"
                else:
                    desc = f"{label} - سن رقم {active_num}"
                db.add_transaction(self.patient_id, "charge", record_price, description=desc,
    ...
    def _open_tooth_annotation_dialog(self, tooth_num):
        ...
        def delete_note():
            if not messagebox.askyesno("تأكيد الحذف", "هل تريد حذف هذه الملاحظة نهائيًا؟", parent=dialog):
                return
