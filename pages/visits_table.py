...
SERIAL_KEY = "serial"
SERIAL_LABEL = "م"
# كان 34 بيكسل بس - ده كان بيقطع رقم المسلسل و/أو خانة التاريخ جنبه لما
# الجدول يتضيّق (النص بيتقطع من غير أي مؤشر "..." لأن CTkLabel مش بتعمل
# تصغير أو التفاف تلقائي للنص). كبّرناه شوية لمساحة كافية دايمًا لرقمين
SERIAL_COL_MINSIZE = 40

ACTION_COL_MINSIZE = 66
N_DATA_COLS = len(COLUMNS)
N_READING_COLS = N_DATA_COLS + 1  # +1 لعمود المسلسل
# الحد الأدنى المطلق اللي أي عمود متمدد (التاريخ/الطبيب/الملاحظات) ممكن
# يتقلّص له وقت ضيق المساحة - كان 34 بيكسل وده مكنش كفاية لعمود التاريخ
# (10 حروف/أرقام "٢٠٢٦-٠٨-١٧") فكان بيتقطع، فكبّرناه لمساحة كافية دايمًا
MIN_COL_FLOOR = 50
...
class PatientVisitsTable(ctk.CTkFrame):
    ...
    def _current_col_pixel_width(self, col):
        """بترجع العرض الفعلي المعروض دلوقتي (بالبكسل) لعمود معيّن من
        الجدول - بنقرأه من صف الهيدر (row=0) لأنه موجود دايمًا لكل الأعمدة
        بعكس صفوف البيانات اللي ممكن ما توجدش. بنستخدمها بدل self._col_widths
        وقت التجميد لأن الأخيرة بتفضل مخزّنة بالعرض الطبيعي الأساسي بس
        (زي 220px لعمود الملاحظات) حتى لو الجدول كان بيتمدد فعليًا لعرض
        أكبر بكتير من ده (بسبب وزن التمدد العالي بتاعه)"""
        try:
            bbox = self._table.grid_bbox(row=0, column=col)
        except Exception:
            bbox = None
        return bbox[2] if bbox else None

    def _freeze_all_columns(self, active_grid_col, active_width):
        """بتجمّد كل أعمدة الجدول (مش بس العمود اللي بيتفتح للتعديل) على
        مقاساتها الحالية بالظبط (weight=0) طول فترة التعديل، عشان مفيش أي
        عمود يتمدد ويشغل مساحة العمود المجمّد. بترجع الإعداد الطبيعي
        القابل للتمدد تاني _apply_columns بعد التأكيد أو الإلغاء.

        بنقرأ العرض الفعلي الحقيقي لكل عمود (مش بس العمود النشط) قبل أي
        تعديل على إعدادات الأعمدة، عشان القراءة تكون من نفس التخطيط
        المتمدد الأصلي - لو استخدمنا self._col_widths هنا (اللي بتفضل
        بالعرض الطبيعي الأساسي الصغير حتى وقت التمدد) كان بيحصل انهيار
        مفاجئ في عرض الأعمدة التانية (خصوصًا عمود "ما تم في الزيارة" اللي
        وزن تمدده أعلى وزن) وتفضل مساحة فاضية كبيرة في الجدول"""
        try:
        ...
        for i, (key, _label, _minsize, _weight, _kind) in enumerate(COLUMNS):
            col = self._grid_col(i + 1)
            if col != active_grid_col:
                actual_widths[key] = self._current_col_pixel_width(col)

        self._table.grid_columnconfigure(self._action_col(), weight=0, minsize=ACTION_COL_MINSIZE)
        self._table.grid_columnconfigure(self._grid_col(0), weight=0, minsize=SERIAL_COL_MINSIZE)
        for i, (key, _label, minsize, _weight, _kind) in enumerate(COLUMNS):
            col = self._grid_col(i + 1)
            if col == active_grid_col:
                self._table.grid_columnconfigure(col, weight=0, minsize=active_width)
            else:
                w = actual_widths.get(key) or self._col_widths.get(key, minsize)
                self._table.grid_columnconfigure(col, weight=0, minsize=w)
    ...
    def _confirm_delete(self, record_id):
        ...
        if not messagebox.askyesno(
                "تأكيد الحذف",
                "هل تريد حذف هذه الزيارة نهائيًا؟",
                parent=self):
            return
    ...
    def _start_edit(self, row_index, record, key, kind, grid_col, row_bg):
        ...
        # نجمّد عرض كل الأعمدة (مش بس العمود اللي بيتفتح للتعديل) على
        # مقاساتها الحالية بالظبط. لو جمّدنا عمود واحد بس (weight=0) وسبنا
        # باقي الأعمدة على وضعها الطبيعي (بأوزانها العادية)، فالمساحة
        # الإضافية اللي كانت المفروض تروح للعمود المجمّد بتتوزع بالكامل
        # على باقي الأعمدة اللي لسه ليها weight> 0 فتتمدد هي وتكبر بشكل
        # واضح (زي ما كان بيحصل: عمود "الطبيب" و"التاريخ" بيكبروا جدًا لما
        # نفتح عمود "ما تم في الزيارة" للتعديل، لأنه أكبر وزن تمدد أصلاً).
        # الحل: نجمّد كل الأعمدة سوا على نفس مقاساتها الحالية بالظبط طول
        # فترة التعديل، وترجع لوضعها الطبيعي (القابل للتمدد) بعد التأكيد
        # أو الإلغاء عن طريق _apply_columns
        self._freeze_all_columns(grid_col, cell_width)
